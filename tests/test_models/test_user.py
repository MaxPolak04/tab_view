...
def test_create_user(init_database):
    ...
    """
    user = User(username="testuser", password="hashedpassword", is_admin=False)  # nosec
...
def test_user_repr(init_database):
    ...
    """
    user = User(username="john", password="secret")  # nosec
    init_database.session.add(user)
    init_database.session.commit()
...
def test_user_requires_username(init_database):
    ...
    """
    user = User(password="secret")  # nosec
    init_database.session.add(user)
...
def test_username_must_be_unique(init_database):
    ...
    """
    user1 = User(username="admin", password="123")  # nosec
    user2 = User(username="admin", password="456")  # nosec
...
def test_user_is_not_admin_by_default(init_database):
    ...
    """
    user = User(username="normal", password="123")  # nosec
    init_database.session.add(user)
    init_database.session.commit()
...
def test_user_created_at_is_set(init_database):
    ...
    """
    user = User(username="time", password="123")  # nosec
    init_database.session.add(user)
    init_database.session.commit()
