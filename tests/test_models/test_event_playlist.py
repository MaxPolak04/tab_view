...
def test_event_to_dict_returns_sorted_media_playlist(init_database):
    ...
    device = Device(name="N.200", device_url="n200")

    # Link media to the tag
    media1 = Media(filename="video.mp4", media_type="video", tag_id=tag.id)
    media2 = Media(filename="image.jpg", media_type="image", tag_id=tag.id)
    ...
    em2 = EventMedia(event=event, media=media2, order=2, duration=5)

    em1 = EventMedia(event=event, media=media1, order=1, duration=10)
