...
from flask import render_template, url_for, redirect, session, flash, current_app, request
...
def signin():
    ...
    if form.validate_on_submit():
        ...
        if not check_password_hash(user.password, password):
            logger.warning(f"Login failed - invalid password for user '{username}' (ID: {user.id}) from IP: {client_ip}")
        ...
        logger.info(f"User logged in successfully: {user.username} (ID: {user.id}) from IP: {client_ip}")
